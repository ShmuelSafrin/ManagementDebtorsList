	...
	while (head != NULL)
	{
		...
		else
		{
			printf("|%-15s %-15s %-15s %-15s %-15.2f %-15s   |\n", head->FirstName, head->LastName,
				head->ID, head->PhoneNumber, head->Debt, head->Date);
			printf("----------------------------------------------------------------------------------------------------\n");
			head = head->next;
		}
	}
			...
			else if (!strcmp(field, "debt"))
			{
				if (isCorrectAmount(word))
				{
					...
					else if (!strcmp(operator, ">"))
					{
						while (current != NULL)
						{
							if (current->Debt > atof(word))/*if return 1. So current->FirstName > word*/
								buildNodeQuery(&headQuery, &tailQuery, current);
							current = current->next;
						}
						if (headQuery == NULL)
			...
			else if (!strcmp(field, "date"))
			{
				if (checkDateValidation(word))
				{
					...
					if (!strcmp(operator, "="))
					{
						while (current != NULL)
						{
							if (!strcmp(current->Date, word))/*if 0. i.e they are equal*/
								buildNodeQuery(&headQuery, &tailQuery, current);
							current = current->next;
						}
						if (headQuery == NULL)
					...
					else if (!strcmp(operator, "!="))
					{
						while (current != NULL)
						{
							if (strcmp(current->Date, word))/*if not 0. i.e they are not equal*/
								buildNodeQuery(&headQuery, &tailQuery, current);
							current = current->next;
						}
						if (headQuery == NULL)
			...
			if (!strcmp(operator, "=") || !strcmp(operator, "!=") || !strcmp(operator, "<") || !strcmp(operator, ">") || !strcmp(operator, "<=") || !strcmp(operator, ">="))
			{
				...
				if (checkSizeAndDigits(word, 10))
				{
					...
					if (!strcmp(operator, "="))
					{
						while (current != NULL)
						{
							if (!strcmp(current->PhoneNumber, word))/*if 0. i.e they are equal*/
								buildNodeQuery(&headQuery, &tailQuery, current);
							current = current->next;
						}
						if (headQuery == NULL)
					...
					else if (!strcmp(operator, "!="))
					{
						while (current != NULL)
						{
							if (strcmp(current->PhoneNumber, word))/*if not 0. i.e they are not equal*/
								buildNodeQuery(&headQuery, &tailQuery, current);
							current = current->next;
						}
						if (headQuery == NULL)
