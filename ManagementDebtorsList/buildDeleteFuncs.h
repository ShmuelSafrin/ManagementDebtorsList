...
the pointer to the linked list and the number of line
of the file(which is zero at the begining)
to notify user, if problemtic field will be found
in which line it is. The function gets
...
the function turns to the addFileNodeToList
function, which inserts the
node that it receives into the linked list.*/
void fromFileToList(FILE*, Debtor**, int*);

/*This function receives the new node from function
fromFileToList. and checks if such a node
...
void addFileNodeToList(Debtor**, Debtor*, int*);
...
buildListQuery function sends it the new node
to link it to the link list*/
void buildNodeAndListQuery(Debtor**, Debtor**, Debtor*);

/*The function gets head and tail and the new node
created by the function buildNodeAndListQuery
...
/*In fact this function is very similar to the function fromFileToList
...
void setUserDebtorToList(FILE*, Debtor**, int*);

/*In fact this function is very similar to the function addFileNodeToList
This function gets its arguments from the function setUserDebtorToList
Unlike the function addFileNodeToList it checks where to connect the new node
